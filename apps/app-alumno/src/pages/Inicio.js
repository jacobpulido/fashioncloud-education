...
function dueLabel(fecha) {
    ...
    if (d < 0)
        return { txt: 'Vencida', soon: true };
    if (d === 0)
        return { txt: 'Hoy', soon: true };
    if (d <= 3)
        return { txt: `En ${d} dias`, soon: true };
    return { txt: new Date(fecha).toLocaleDateString('es-MX', { day: 'numeric', month: 'short' }), soon: false };
}
function TabIcon({ tab, active }) {
    const svg = (path) => <path d={path}/>;
    const circles = (cx, cy, r) => <circle cx={cx} cy={cy} r={r}/>;
    let content;
    if (tab === 'Inicio') {
    ...
    return (<div className={`tab ${active ? 'active' : ''}`}>
      ...
      <div className="ph-scroll">
        ...
        <div className="ph-pad">
          ...
          {actividades.map((act) => {
            ...
            return (<div key={act.id} className="todo" onClick={() => navigate(`/actividad/${act.id}`)}>
