...
import { getMisMaterias, getActividades } from '../lib/api';
...
interface ActividadUI {
  id: string; tipo: string; titulo: string; materia: string; fecha: string; cover: number;
}
...
function TabIcon({ tab, active }: { tab: string; active: boolean }) {
  ...
  if (tab === 'Inicio') content = <path d="M3 9.5 12 3l9 6.5V20a1 1 0 0 1-1 1h-5v-7H9v7H4a1 1 0 0 1-1-1z" />;
  else if (tab === 'Avances') content = <>{circles(12, 8, 6)}<path d="M15.5 13.5 17 22l-5-3-5 3 1.5-8.5" /></>;
  else content = <><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" />{circles(12, 7, 4)}</>;
  return (<div className={`tab ${active?'active':''}`}><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">{content}</svg><span>{tab}</span></div>);
}

export function Inicio() {
  const navigate = useNavigate();
  const { user, logout } = useAuth();
  const [items, setItems] = useState<ActividadUI[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    (async () => {
      try {
        const materias: any[] = await getMisMaterias();
        const all: ActividadUI[] = [];
        let idx = 0;
        for (const m of materias) {
          const acts: any[] = await getActividades(m.id);
          for (const a of acts) {
            if (a.estado === 'borrador') continue;
            all.push({
              id: a.id, tipo: a.tipo, titulo: a.titulo,
              materia: m.nombre, fecha: a.fecha_limite || '',
              cover: idx++ % COVERS.length,
            });
          }
        }
        setItems(all);
      ...
      <div className="ph-scroll">
        <div className="ph-header grad">
          ...
          <div className="ph-name">
            <span>{user?.nombre || 'Usuario'} {user?.apellidos || ''}</span>
            <div style={{ width:38, height:38, borderRadius:'50%', background:'#E11D6B', display:'grid', placeItems:'center', fontSize:14, fontWeight:600, color:'#fff' }}>
              {initials}
            </div>
          </div>
        </div>
        ...
        <div className="ph-pad">
          <div className="ph-section-t">
            ...
            <span style={{ color: 'var(--magenta)' }}>{items.length}</span>
          </div>

          <button onClick={logout} style={{background:'none',border:'none',color:'var(--muted)',cursor:'pointer',marginBottom:12,fontSize:13}}>Cerrar sesión</button>

          {loading && <p style={{color:'var(--muted)',textAlign:'center',padding:20}}>Cargando actividades…</p>}

          {!loading && items.length === 0 && <p style={{color:'var(--muted)',textAlign:'center',padding:20}}>No hay actividades pendientes 🎉</p>}
