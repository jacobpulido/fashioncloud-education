...
import { MobileNavAlumno } from "./mobile-nav";
    ...
    <div className="flex min-h-screen">
      <aside className="fixed inset-y-0 left-0 hidden w-64 bg-[#0b1120] px-4 py-6 lg:block">
        <div className="mb-8 px-3">
          ...
          <p className="mt-0.5 text-xs text-white/50">{user.user_metadata?.nombre || "Alumno"}</p>
        </div>
        <nav className="space-y-1">
          <Link href="/alumno/pendientes" className="flex items-center gap-3 rounded-md px-3 py-2 text-sm text-white/70 transition hover:bg-white/10">
            <span>Mis pendientes</span>
          </Link>
          <Link href="/alumno/materias" className="flex items-center gap-3 rounded-md px-3 py-2 text-sm text-white/70 transition hover:bg-white/10">
      ...
      <MobileNavAlumno nombre={user.user_metadata?.nombre} />
