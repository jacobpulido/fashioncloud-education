...
export async function procesarConIA(
  ...
  if (!texto || texto.trim().length < 20) return { status: "error", message: "El texto debe tener al menos 20 caracteres." };
  try {
...
export async function guardarMateria(data: MateriaData): Promise<{ ok: boolean; error?: string }> {
  ...
  const { error } = await supabase.from("materias_carga").insert({
    docente_id: user.id, nombre: data.nombre, descripcion: data.descripcion,
    color: data.color, plan_estudios: data.plan_estudios, estado: "borrador",
  });
  if (error) return { ok: false, error: error.message };
  revalidatePath("/materias"); revalidatePath("/dashboard");
  return { ok: true };
}
...
export async function actualizarEstadoMateria(id: string, estado: string) {
  const supabase = await createClient();
  const { error } = await supabase
    ...
    .eq("id", id);
  if (error) return { ok: false, error: error.message };
  revalidatePath("/materias/" + id); revalidatePath("/materias"); revalidatePath("/dashboard");
  return { ok: true };
}
